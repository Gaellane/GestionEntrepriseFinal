...
const emptyRow = (type) => ({ type: type || 'ENTREE', articleId: '', depotId: '', quantite: '', description: '', date: '', datePeremption: '' });

export default function MvtStockSaisie() {
  const { type } = useParams();
  // accept numeric type (1=ENTREE,2=SORTIE) or string 'ENTREE'/'SORTIE'
  let defaultType;
  if (!type) {
    defaultType = 'ENTREE';
    ...
    defaultType = (t === 'SORTIE') ? 'SORTIE' : 'ENTREE';
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (const r of rows) {
        if (!r.articleId || !r.quantite || !r.depotId) {
          throw new Error('Chaque ligne doit contenir article, quantité et dépôt');
        }

        const payload = {
          type: activeTab === 'ENTREE' ? 1 : 2,
                  ...
                  <div key={idx} className="border rounded-lg p-4 mb-3">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <select value={row.depotId} onChange={e => updateRow(idx, 'depotId', e.target.value)} className="mt-1 w-full px-3 py-2 border rounded-lg">
                          <option value="">-- Choisir --</option>
